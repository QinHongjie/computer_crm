...
<head>
    ...
    <link href="${pageContext.request.contextPath }/static/bootstrap-switch/css/bootstrap3/bootstrap-switch.css" rel="stylesheet">
	<script src="${pageContext.request.contextPath }/static/bootstrap-switch/js/bootstrap-switch.js"></script>
...
<body>
		...
		<table class="table table-striped table-hover">
	 		...
	 		<c:forEach items="${userList }" var="u">
	 			<tr>
		 			...
		 			<td><input id="a_status${u.id}" class="${u.ident==1?'a_true':''} ${ADMIN_SESSION.id==u.id||u.status==0?'read':''} admin_status" type="checkbox" value="${u.id}"/></td>
			 	</tr>
			</c:forEach>
		</table>
	</div>
	...
	<nav id="paging" aria-label="Page navigation">
		<ul id="page_both" class="pager">
			...
			<c:otherwise>
				<li class="previous"><a href="user_right.action?page=${page-1}">
			...
			<c:otherwise>
				<li class="next"><a href="user_right.action?page=${page+1}">
		...
		<ul id="page_num" class="pagination">
			<c:forEach begin="${page-5>0?page-5:1 }" end="${page+4>sumPage?sumPage:page+4 }" var="n">
				...
				<c:if test="${page != n}">
					<li><a href="user_right.action?page=${n }">${n }</a></li>
				</c:if>
			</c:forEach>
		</ul>
	</nav>
	...
	<script>
		...
		$(function(){
			...
			$("select").change(function(){
				// 将改变的数据显示数值传给后台做处理
				location.href = "user_right.action?num="+$(this).val();
			})
		})
	...
	                if (state == true) {
						// 打开按钮时触发事件
						var status = "1";
	                  	if(!isUserRightExist("setUserStatus",userid,status)){
	...
	                  	// 打开按钮时触发事件
						var status = "1";
					    if(!isUserRightExist("setUserIdent",userid,status)){
							// 若管理员权限授予失败
