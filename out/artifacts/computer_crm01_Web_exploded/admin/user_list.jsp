...
<body>
	...
	<c:if test="${isRequset != 1 }">
		<script>location.href="user_list.admin";</script>
	</c:if>
		...
		<table class="table table-striped table-hover">
	 		...
	 		<c:forEach items="${userList }" var="u">
	 			<tr>
					...
					<!-- 判断如果是当前登录用户 则不显示信息 -->
	 				<c:if test="${u.userName eq adminName}" >
	...
	<nav id="paging" aria-label="Page navigation">
		<ul id="page_both" class="pager">
			...
			<c:otherwise>
				<li class="previous"><a href="user_list.admin?type=${type }&page=${page-1}">
			...
			<c:otherwise>
				<li class="next"><a href="user_list.admin?type=${type}&page=${page+1}">
		...
		<ul id="page_num" class="pagination">
			<c:forEach begin="${page-5>0?page-5:1 }" end="${page+4>sumPage?sumPage:page+4 }" var="n">
				...
				<c:if test="${page != n}">
					<li><a href="user_list.admin?type=${type }&page=${n }">${n }</a></li>
				</c:if>
			</c:forEach>
		</ul>
	</nav>
	...
	<script>
		...
		$(function(){
			...
			$("select").change(function(){
				location.href = "user_list.admin?num="+$(this).val();
			})
		})
	</script>
