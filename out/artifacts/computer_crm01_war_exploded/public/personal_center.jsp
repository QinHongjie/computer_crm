...
<head>
	...
	<link href="${pageContext.request.contextPath}/static/css/path_nav.css" rel="stylesheet">
	<script src="${pageContext.request.contextPath}/static/js/userinfoCheck.js"></script>
	<script src="${pageContext.request.contextPath}/static/js/alterUserInfo.js"></script>
	<style>
		#per_center div{
			background: white;
		}
	...
	<div id="per_center">
		<div class="container">
			<div id="head_photo" class="col-md-5 col-sm-12 text-center">
				<img id="showImg" src="${pageContext.request.contextPath }/static/img/head_photo.jpg" />
			...
			<div class="col-md-7 col-sm-12">
				<!-- 修改个人信息表单域 -->
				<form id="per_form" class="form-signin" action="user_alter_info.do" method="post" onsubmit="return false">
					<!-- 用户名输入区 -->
					<label>用户名：</label>
					<input name="alterName" type="text" id="inputAlterName" class="form-control input" value="${user.get('userName') }" placeholder="昵称(1~8位中文或英文字符)" maxlength="8" required />
					<label for="name" class="alert alert-danger">验证用户名错误提示信息</label>
					<!-- 电话号码输入区 -->
					<label>电话号码：</label>
					<input name="alterTel" type="tel" id="inputTel" class="form-control input" value="${user.get('telephone') }" placeholder="电话号码" required>
					<label for="tel" class="alert alert-danger">验证用户名错误提示信息</label>
					<!-- 收货地址输入区 -->
					<label><font color="red">*</font>收货地址：</label>
					<input name="alterAddress" type="address" id="inputAddress" class="form-control input" value="${user.get('address') }" placeholder="收货地址" required>
				...
				<!-- 修改密码文本域 -->
				<form id="pass_form" class="form-signin" action="user_alter_pass.do" method="post" onsubmit="return false">
					<!-- 原密码输入区 -->
					<label>原密码：</label>
					...
					<img class="loading" src="<%=request.getContextPath() %>/static/img/loading.gif" width="24px" />
				</form>
			</div>
		</div>
	</div>
