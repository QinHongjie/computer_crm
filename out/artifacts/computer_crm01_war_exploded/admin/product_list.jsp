...
<body>
	...
	<c:if test="${isRequset != 1 }">
		<script>location.href="product_list.admin";</script>
	</c:if>
	...
	<div class="table-responsive">
		<table class="table table-striped table-hover">
	 		...
	 		<!-- 遍历展示用户信息 -->
	 		<c:forEach items="${pageComs }" var="p">
	 			<tr>
	...
	<nav id="paging" aria-label="Page navigation">
		<ul id="page_both" class="pager">
			...
			<c:otherwise>
				<li class="previous"><a href="product_list.admin?type=${type }&page=${page-1}">
			...
			<c:otherwise>
				<li class="next"><a href="product_list.admin?type=${type}&page=${page+1}">
		...
		<ul id="page_num" class="pagination">
			<c:forEach begin="${page-5>0?page-5:1 }" end="${page+4>sumPage?sumPage:page+4 }" var="n">
				...
				<c:if test="${page != n}">
					<li><a href="product_list.admin?type=${type }&page=${n }">${n }</a></li>
				</c:if>
			</c:forEach>
		</ul>
	</nav>
	...
	<script>
		...
		$(function(){
			...
			$("select").change(function(){
				location.href = "product_list.admin?num="+$(this).val();
			})
		})
	</script>
</body>
